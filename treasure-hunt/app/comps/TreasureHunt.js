...
const TreasureHunt = () => {
  const [treasureLocation, setTreasureLocation] = useState(Math.floor(Math.random() * 9));
  const [bombLocation, setBombLocation] = useState(Math.floor(Math.random() * 9));
  ...
  return (
    <> 
    <div className='text-center bg-gradient-to-tl from-slate-700 via-slate-800 to-slate-700 m-10 rounded-xl border-4 border-cyan-500 text-white font-mono'>
